...
//	keys[KEY_RIGHT]->set(SDLK_l);
	keys[KEY_DBG_ACCEPT1]->set(SDLK_RETURN);
	keys[KEY_DBG_ACCEPT2]->set(SDLK_SPACE);
	keys[KEY_DBG_RIGHT]->set(SDLK_RIGHT);
	...
	keys[KEY_SPELLS]->set(SDLK_m);
	keys[KEY_INVENTORY]->set(SDLK_i);
	keys[KEY_INTERACT]->set(SDLK_o);
	keys[KEY_SEARCH]->set(SDLK_s);
	keys[KEY_OPTIONS]->set(SDLK_w);
	keys[KEY_QUIT]->set(SDLK_q);
	keys[KEY_RUN]->set(SDLK_LSHIFT);
	keys[KEY_DBG_QUIT]->set(SDLK_ESCAPE);
	keys[KEY_DBG_SHOT]->set(SDLK_F12);
	keys[KEY_DBG_1]->set(SDLK_F1);
	keys[KEY_DBG_2]->set(SDLK_F2);
}
...
//	printf("name: %s\n", pElem->Value());
	...
	pElem=hRoot.FirstChild( "keys" ).FirstChild("spells").Element();
	if (!pElem) return false;
	pElem->QueryIntAttribute("value",&key);
	keys[KEY_SPELLS]->set(key);
	...
	pElem=hRoot.FirstChild( "keys" ).FirstChild("search").Element();
	if (!pElem) return false;
	pElem->QueryIntAttribute("value",&key);
	keys[KEY_SEARCH]->set(key);
